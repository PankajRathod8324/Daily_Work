...
public class AccountManagerOrderAppService : IAccountManagerOrderAppService
{
    ...
    public List<MenuCategoryVM> SetOrderItemDataWithId(int CustomerId)
    {
        ...
        foreach (var item in orderitem)
        {
            var itemdata = new MenuCategoryVM
            {
                ...
                ItemName = _menuRepository.GetItemById((int)item.ItemId).ItemName,
                Rate = _menuRepository.GetItemById((int)item.ItemId).Rate,
                ItemInstruction = item.ItemInstructions,
                Quantity = item.Quantity,
                UId = item.Uid,
                MenuItemModifier = _orderRepository.GetOrderModifiersByOrderItemIdAndItemId(item.OrderItemId, (int)item.ItemId).Select(mod => new MenuModifierGroupVM
                {
                    ModifierId = (int)mod.ModifierId,
                    ModifierName = _menuRepository.GetModifierNameById((int)mod.ModifierId),
                    ModifierRate = _menuRepository.GetModifierById((int)mod.ModifierId).ModifierRate,
                }).ToList(),
            };
    ...
    public bool SaveOrUpdateOrder(List<MenuCategoryVM> orderDetailsArray)
    {
        ...
        if (existorder != null)
        {
            ...
            existorder.NoOfPerson = _customerRepository.GetNoOfPersons((int)orderDetailsArray[0].CustomerId);
            ...
            foreach (var existingItem in existingOrderItems)
            {
                ...
                var modifierNewList = _orderRepository.GetOrderModifiersByOrderItemIdAndItemId(existingItem.OrderItemId, (int)existingItem.ItemId);
        ...
        else
        {
            var newOrder = new Order
            {
                ...
                NoOfPerson = _customerRepository.GetNoOfPersons((int)orderDetailsArray[0].CustomerId),
            ...
            var table = _accountmanagerorderapprepository.GetTablesByActiveCustomerId((int)orderDetailsArray[0].CustomerId);
            foreach (var tb in table)
            {
                var ordertable = new OrderTable
                {
                    OrderId = orderId,
                    TableId = (int)tb.TableId,
                };
                _accountmanagerorderapprepository.AddOrderTable(ordertable);
            }

        }
        var tables = _accountmanagerorderapprepository.GetTablesByActiveCustomerId((int)orderDetailsArray[0].CustomerId);

        foreach (var table in tables)
        {
            var selectedtable = _tableRepository.GetTableById((int)table.TableId);
    ...
    {
        var order = _orderRepository.GetOrderById((int)review.OrderId);
        if (order != null)
        {
